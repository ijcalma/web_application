<?php
  ...
  if(isset($_POST['submit'])){
    ...
    // Fetching the half-day type of the student from the events table
    ...
    if ($is_morning && $half_day_type != 'Afternoon') {
      $query_check_existing = "SELECT * FROM attendance WHERE student_id = '$idnum' AND eventid = '$eventid' AND timeout_am IS NOT NULL";
      $result_check_existing = mysqli_query($conn, $query_check_existing);

      if (mysqli_num_rows($result_check_existing) > 0) {
              echo "<script>alert('An existing record with the same student ID and event ID already has a time-out entry.');</script>";
            }
      else{
          $query = "INSERT INTO attendance (student_id, eventid, timeout_am)
            VALUES ('$idnum', '$eventid', '$date')
            ON DUPLICATE KEY UPDATE timeout_am = '$date'";
          ...
          if ($result3) {
            ...
            if(mysqli_query($conn, $query)){
              ...
              if(mysqli_query($conn, $attendance_check)){
                ...
                $update_attendance = "UPDATE event_attendance SET timein_no = $timein_no, timeout_no = $timeout_no WHERE studentid = '$idnum' AND eventid = '$eventid'";
            ...
            $query_check_existing = "SELECT * FROM attendance WHERE student_id = '$idnum' AND eventid = '$eventid' AND timeout_pm IS NOT NULL";
            $result_check_existing = mysqli_query($conn, $query_check_existing);
            ...
            $query = "INSERT INTO attendance (student_id, eventid, timeout_pm)
            VALUES ('$idnum', '$eventid', '$date')
            ON DUPLICATE KEY UPDATE timeout_pm = '$date'";
